...
if [ -z "$network" ];
	then
		sudo docker network create --dirver overlay caching_network
	else
		echo "+ Network Exist"
fi
...
sudo docker -H :4000 stop  dc-server1 dc-server2 dc-server3 dc-server4 dc-client
sudo docker -H :4000 rm -f dc-server1 dc-server2 dc-server3 dc-server4 dc-client

sudo docker -H :4000 run --name dc-server1 --network caching_network -d cloudsuite/data-caching:server -t 4 -m 4096 -n 550
sudo docker -H :4000 run --name dc-server2 --network caching_network -d cloudsuite/data-caching:server -t 4 -m 4096 -n 550
sudo docker -H :4000 run --name dc-server3 --network caching_network -d cloudsuite/data-caching:server -t 4 -m 4096 -n 550
sudo docker -H :4000 run --name dc-server4 --network caching_network -d cloudsuite/data-caching:server -t 4 -m 4096 -n 550
