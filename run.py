...
def deploy_benchmark(action):
    ...
    if action == "create":
        ...
        logging.warning("The action is not defined. please use create or delete")
        sys.exit()
...
def metadata():
        ...
        logging.info("        Number of Server: {}            ".format(args.server_no))
        logging.info("        Server Threads:   {}            ".format(args.server_threads))
        logging.info("        dedicated memory: {}            ".format(args.memory))
        logging.info("        Object Size:      {}            ".format(args.object_size))
        logging.info("        --------- Client ----------")
        logging.info("        Client threats:   {}            ".format(args.client_threats))
        logging.info("        Interval:         {}            ".format(args.interval))
        logging.info("        Server memory:    {}            ".format(args.server_memory))
        logging.info("        Scaling factor:   {}            ".format(args.scaling_factor))
        logging.info("        Fraction:         {}            ".format(args.fraction))
        logging.info("        Connections:      {}            ".format(args.connection))
        logging.info("        Duration:         {}            ".format(args.duration))
        logging.info("\n")
